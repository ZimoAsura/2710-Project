...
def cartData(request, salesId):
	if request.user.is_authenticated:
		customer = request.user.customer			
		sales = Sales.objects.get(id=salesId)
		# order = Order.objects.filter(customer=customer, sales = sales, complete=False)
	...
	return {'cartItems':cartItems ,'order':order, 'items':items, 'sales':sales}
